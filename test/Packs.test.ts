...
import { PacksFacet } from '../typechain';
...
describe("Packs Test", function() {
    ...
    before(async () => {
      ...
      const diamond = await deploy.deployDiamond(
        'Packs',
        [cutFacet, loupeFacet, ownershipFacet, packsFacet],
        userAddress,
      );

      packs = (await diamondAsFacet(diamond, 'PacksFacet')) as PacksFacet;
    });
  
    it("should create collectible", async function() {
      const fees = [{
        recipient: randomWallet1.address,
        value: feeSplit1
      }, {
      ...
      await packs.connect(user).addCollectible(metadata[0].coreData, metadata[0].assets, metadata[0].secondaryAssets, metadata[0].metaData, fees);
    });
